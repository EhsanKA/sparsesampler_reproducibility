...
"""
Generate UMAP visualizations comparing different sampling methods in MCC reference dataset.
...
class UMAPVisualizer:
    ...
    def __init__(self, data_path: str, directory: str = "mcc/benchmark", test_mode: bool = False, figures_dir: str = "figures"):
    ...
    def generate_individual_umap(self, adata: sc.AnnData, method: str):
        """Generate UMAP plot for Figure 1 (MCC) with three square panels (sps, rare, random)."""
        filename = 'test_figure1_mcc_sampling_comparison.jpg' if self.test_mode else 'figure1_mcc_sampling_comparison.jpg'
    ...
    def generate_combined_umaps(self, adata: sc.AnnData, method: str):
        ...
        filename = 'test_supp_figure2_mcc_sampling_analysis.jpg' if self.test_mode else 'supp_figure2_mcc_sampling_analysis.jpg'
        output_file = os.path.join(self.figures_dir, filename)
        os.makedirs(os.path.dirname(output_file), exist_ok=True)
    ...
    def generate_label_umap(self, adata: sc.AnnData):
        ...
        filename = 'test_supp_figure1_mcc_cell_types.jpg' if self.test_mode else 'supp_figure1_mcc_cell_types.jpg'
        output_file = os.path.join(self.figures_dir, filename)
        os.makedirs(os.path.dirname(output_file), exist_ok=True)
...
def main():
    parser = argparse.ArgumentParser(description='Generate UMAP visualizations for MCC data')
