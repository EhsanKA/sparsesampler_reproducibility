...
"""
Generate UMAP visualizations comparing different sampling methods (sps and random) in a reference dataset.
...
class UMAPVisualizer:
    ...
    def __init__(self, data_path: str, directory: str = "lcmv/benchmark", test_mode: bool = False, figures_dir: str = "figures"):
        ...
        self.OBS_FEATURES = ['prediction', 'organ', 'sample_group', 'label', 'group', 'celltype', 'sample_id']
        self.DROP_FEATURES = ['SSC-B-H', 'SSC-B-A']
    ...
    def identify_rare_cell_types(self, adata: sc.AnnData) -> sc.AnnData:
        ...
        real_counts = adata.obs[self.label_key].value_counts()
    ...
    def load_sampling_results(self, ref: int, method: str, size: int, rep: int) -> np.ndarray:
        """Load sampling results from pickle file or generate random indices in test mode."""
        if self.test_mode:
    ...
    def generate_individual_umap(self, adata: sc.AnnData, method: str):
        """Generate UMAP plot for Figure 1 (LCMV) with three square panels (sps, rare, random), each with its legend at the bottom in a single row."""
        filename = 'test_figure1_lcmv_sampling_comparison.jpg' if self.test_mode else 'figure1_lcmv_sampling_comparison.jpg'
        output_file = os.path.join(self.figures_dir, filename)
        os.makedirs(os.path.dirname(output_file), exist_ok=True)
        # Make square panels: 3x (4x4) panels, extra height for legends
        ...
        for i, color_key in enumerate(self.COLORS_TO_PLOT):
            ...
            ax.set_aspect('equal')  # Make axes square
            ...
            ax.legend(
                ...
                loc='upper center',
                bbox_to_anchor=(0.5, -0.28),  # move legend further down
                ncol=len(categories),
                frameon=False,
                handletextpad=self.LEGEND_HANDLE_TEXTPAD,
                labelspacing=self.LEGEND_LABEL_SPACING,
            )
        plt.tight_layout(rect=[0, 0.22, 1, 1])  # more space for legends
    ...
    def generate_combined_umaps(self, adata: sc.AnnData, method: str):
        ...
        filename = 'test_supp_figure2_lcmv_sampling_analysis.jpg' if self.test_mode else 'supp_figure2_lcmv_sampling_analysis.jpg'
        output_file = os.path.join(self.figures_dir, filename)
        os.makedirs(os.path.dirname(output_file), exist_ok=True)
    ...
    def generate_label_umap(self, adata: sc.AnnData):
        ...
        filename = 'test_supp_figure2_lcmv_cell_types.jpg' if self.test_mode else 'supp_figure2_lcmv_cell_types.jpg'
        output_file = os.path.join(self.figures_dir, filename)
        os.makedirs(os.path.dirname(output_file), exist_ok=True)

        fig, ax = plt.subplots(figsize=(10, 10))  # Large square canvas
        ...
        # Place the legend below the plot, with more columns and larger font
        ax.legend(
            fontsize=20,
            ncol=4,  # Adjust as needed for your number of cell types
            frameon=False,
            loc="upper center",
...
def main():
    parser = argparse.ArgumentParser(description='Generate UMAP visualizations for LCMV data')
