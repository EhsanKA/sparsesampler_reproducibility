...
bash run_all_feature_indices.sh && python ../combine_results.py --dataset mcc

# MCC_01 / MCC_05
bash run_mcc01_mcc05_slurm.sh mcc_01 && python ../combine_results.py --dataset mcc_01
bash run_mcc01_mcc05_slurm.sh mcc_05 && python ../combine_results.py --dataset mcc_05

# LCMV
cd jobs/feature_index_classification_lcmv/
bash run_all_feature_indices_lcmv.sh && python ../combine_results.py --dataset lcmv
```
...
After sampling benchmarks finish:

```bash
cd analysis/downstream/
bash submit_all.sh          # bins, leiden, proportions, quality
python summarize.py         # merge task outputs + recall table
python plot_figures.py      # Fig. S15
```

Optional jobs: `bash submit.sh {adjacent|within_pop_precompute|within_pop}`

Key outputs:
- `results/discovery_rate_summary.csv`, `per_celltype_recall_summary.csv`
- `results/proportion_jsd_summary.csv`, `within_pop_summary.csv`
- `figures/leiden_resolution_lcmv.png`, `adjacent_bin_distribution_*.png`
...
│   └── downstream/                          # Leiden/QC downstream analyses
│       ├── leiden_discovery.py, proportions.py, adjacent_bins.py
│       ├── bins.py, within_pop.py
│       ├── summarize.py, plot_figures.py
│       ├── worker.sh, submit.sh, submit_all.sh
│       ├── results/, figures/
...
│   └── mcc_05/parallel.py
├── jobs/
│   ├── make_tables.py                         # Feature-index + sampling-method tables
│   ├── combine_results.py                     # Merge RF classification CSVs
