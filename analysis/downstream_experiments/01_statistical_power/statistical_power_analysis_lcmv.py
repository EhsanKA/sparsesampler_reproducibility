...
detecting marker genes of rare cell types in the LCMV dataset.
...
- Analyzes all identified rare cell types (not just one hardcoded type)

Hypothesis: SPS sampling captures more cells of rare types, leading to:
- Lower p-values for rare cell type markers (stronger statistical signal)
- Higher log fold changes 
- More markers passing stringent significance thresholds

Usage:
    conda activate facs_sampling
    python statistical_power_analysis_lcmv.py
    
    # Or with custom parameters
    python statistical_power_analysis_lcmv.py --de-method wilcoxon --top-n 50
"""
...
from common.utils import (
    ...
    load_marker_gene_results_lcmv,
    identify_rare_cell_types,
    compute_rare_cell_type_statistics,
...
def load_rare_cell_markers(rare_cell_type, de_method='wilcoxon'):
    """
    Load marker gene results for a rare cell type for SPS and Random sampling.
    
    Parameters:
    -----------
    rare_cell_type : str
        Name of the rare cell type
    de_method : str
        DE method used
    ...
    """
    sps_df = load_marker_gene_results_lcmv('sps', de_method)
    random_df = load_marker_gene_results_lcmv('random', de_method)
    full_df = load_marker_gene_results_lcmv('full', de_method)
    
    # Filter for the rare cell type
    sps_markers = sps_df[sps_df['cell_type'] == rare_cell_type].copy()
    random_markers = random_df[random_df['cell_type'] == rare_cell_type].copy()
    full_markers = full_df[full_df['cell_type'] == rare_cell_type].copy()
    
    return sps_markers, random_markers, full_markers
...
def compute_statistical_power_metrics(sps_markers, random_markers, full_markers, 
    ...
    sps_markers : pd.DataFrame
        SPS rare cell markers
    random_markers : pd.DataFrame
        Random rare cell markers
    full_markers : pd.DataFrame
        Full dataset rare cell markers
    top_n : int
    ...
    # Focus on top N markers (or all if fewer)
    actual_top_n = min(top_n, len(sps_markers), len(random_markers), len(full_markers))
    if actual_top_n == 0:
        return metrics
    
    sps_top = sps_markers.head(actual_top_n)
    random_top = random_markers.head(actual_top_n)
    full_top = full_markers.head(actual_top_n)
    
    # 1. P-value analysis (if available)
    if 'pval' in sps_top.columns and len(sps_top['pval'].dropna()) > 0:
        ...
        if logger:
            logger.info(f"\nP-value analysis (top {actual_top_n} markers):")
    ...
    if 'logfoldchange' in sps_top.columns and len(sps_top['logfoldchange'].dropna()) > 0:
        ...
        if logger:
            logger.info(f"\nLog Fold Change analysis (top {actual_top_n} markers):")
    ...
    if logger:
        logger.info(f"\nScore analysis (top {actual_top_n} markers):")
    ...
    if 'pct_in_group' in sps_top.columns and len(sps_top['pct_in_group'].dropna()) > 0:
        ...
        if logger:
            logger.info(f"\nExpression % analysis (top {actual_top_n} markers):")
...
def compute_marker_overlap(sps_markers, random_markers, full_markers, top_n=50, logger=None):
    ...
    sps_genes = set(sps_markers.head(actual_top_n)['gene'].values)
    random_genes = set(random_markers.head(actual_top_n)['gene'].values)
    full_genes = set(full_markers.head(actual_top_n)['gene'].values)
    ...
    if logger:
        logger.info(f"\nMarker gene overlap (top {actual_top_n}):")
        logger.info(f"  SPS ∩ Random: {overlap['sps_random_overlap']} ({100*overlap['sps_random_overlap']/actual_top_n:.1f}%)")
        logger.info(f"  SPS ∩ Full:   {overlap['sps_full_overlap']} ({100*overlap['sps_full_overlap']/actual_top_n:.1f}%)")
        logger.info(f"  Random ∩ Full: {overlap['random_full_overlap']} ({100*overlap['random_full_overlap']/actual_top_n:.1f}%)")
...
def statistical_tests(metrics, logger=None):
    ...
    if 'pvalue' in metrics and len(metrics['pvalue']['sps_neglogp_values']) > 0:
        sps_vals = metrics['pvalue']['sps_neglogp_values']
        random_vals = metrics['pvalue']['random_neglogp_values']
        
        if len(sps_vals) > 1 and len(random_vals) > 1:
    ...
    if 'score' in metrics and len(metrics['score']['sps_score_values']) > 1:
    ...
    if 'logfoldchange' in metrics and len(metrics['logfoldchange']['sps_lfc_values']) > 1:
...
def plot_pvalue_comparison(metrics, de_method, rare_cell_type, output_dir):
    """Create p-value comparison plots for a rare cell type."""
    ...
    plt.tight_layout()
    safe_name = rare_cell_type.replace('/', '_').replace(' ', '_')
    plt.savefig(os.path.join(output_dir, f'pvalue_comparison_{de_method}_{safe_name}.png'), 
                dpi=150, bbox_inches='tight')
    plt.close()


def plot_volcano_comparison(sps_markers, random_markers, de_method, rare_cell_type, 
                           output_dir, top_n=50):
    ...
    for ax, (df, title, color) in zip(axes, [(sps_markers, 'SPS', '#2ecc71'),
        ...
        # Highlight top genes
        for i, gene in enumerate(df['gene'].head(min(10, len(df))).values):
        ...
        ax.set_ylabel('-log10(p-value)')
        ax.set_title(f'{title} Sample: {rare_cell_type} Markers ({de_method})')
        ax.legend(loc='upper right')
        ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    safe_name = rare_cell_type.replace('/', '_').replace(' ', '_')
    plt.savefig(os.path.join(output_dir, f'volcano_comparison_{de_method}_{safe_name}.png'), 
                dpi=150, bbox_inches='tight')
    plt.close()


def plot_score_comparison(metrics, de_method, rare_cell_type, output_dir):
    """Create score comparison plots."""
    if 'score' not in metrics or len(metrics['score']['sps_score_values']) == 0:
        return
    ...
    plt.tight_layout()
    safe_name = rare_cell_type.replace('/', '_').replace(' ', '_')
    plt.savefig(os.path.join(output_dir, f'score_comparison_{de_method}_{safe_name}.png'), 
                dpi=150, bbox_inches='tight')
    plt.close()


def plot_summary_comparison_all_rare_types(all_results, rare_cell_types, output_dir):
    """Create summary comparison across all rare cell types."""
    # For each DE method, create a summary plot
    de_methods = list(list(all_results.values())[0].keys()) if all_results else []
    
    for de_method in de_methods:
        fig, axes = plt.subplots(1, 3, figsize=(15, 5))
        
        rare_types_with_data = []
        sps_scores = []
        random_scores = []
        sps_neglogp = []
        random_neglogp = []
        ...
        for rare_ct in rare_cell_types:
            if rare_ct in all_results and de_method in all_results[rare_ct]:
                metrics = all_results[rare_ct][de_method]
                if 'score' in metrics:
                    rare_types_with_data.append(rare_ct)
                    sps_scores.append(metrics['score']['sps_mean_score'])
                    random_scores.append(metrics['score']['random_mean_score'])
                    if 'pvalue' in metrics:
                        sps_neglogp.append(metrics['pvalue']['sps_mean_neglogp'])
                        random_neglogp.append(metrics['pvalue']['random_mean_neglogp'])
        
        if not rare_types_with_data:
            plt.close()
            continue
        
        x = np.arange(len(rare_types_with_data))
        width = 0.35
        
        # 1. Mean score comparison
        ax = axes[0]
        ax.bar(x - width/2, sps_scores, width, label='SPS', color='#2ecc71', alpha=0.7)
        ax.bar(x + width/2, random_scores, width, label='Random', color='#e74c3c', alpha=0.7)
        ax.set_ylabel('Mean Score')
        ax.set_title(f'Marker Gene Scores ({de_method})')
        ax.set_xticks(x)
        ax.set_xticklabels(rare_types_with_data, rotation=45, ha='right')
        ax.legend()
        ax.grid(True, alpha=0.3, axis='y')
        
        # 2. Mean -log10(p) comparison (if available)
        ax = axes[1]
        if sps_neglogp:
            ax.bar(x - width/2, sps_neglogp, width, label='SPS', color='#2ecc71', alpha=0.7)
            ax.bar(x + width/2, random_neglogp, width, label='Random', color='#e74c3c', alpha=0.7)
            ...
            ax.set_title('P-values not available for this method')
        
        # 3. Improvement percentage
        ax = axes[2]
        improvements = [(s - r) / r * 100 if r != 0 else 0 
                        for s, r in zip(sps_scores, random_scores)]
        colors = ['#2ecc71' if imp > 0 else '#e74c3c' for imp in improvements]
        ax.bar(x, improvements, color=colors, alpha=0.7)
        ax.axhline(y=0, color='gray', linestyle='-', alpha=0.5)
        ax.set_ylabel('Score Improvement (%)')
        ax.set_title('SPS vs Random Improvement')
        ax.set_xticks(x)
        ax.set_xticklabels(rare_types_with_data, rotation=45, ha='right')
        ax.grid(True, alpha=0.3, axis='y')
        
        plt.suptitle(f'LCMV Rare Cell Types: Statistical Power Comparison ({de_method})', 
                     fontsize=14, y=1.02)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f'summary_comparison_{de_method}.png'), 
                    dpi=150, bbox_inches='tight')
        plt.close()


def create_summary_table(all_results, rare_cell_stats, rare_cell_types, output_dir):
    """Create summary table of results."""
    rows = []
    
    for rare_ct in rare_cell_types:
        if rare_ct not in all_results:
            continue
            
        for de_method, metrics in all_results[rare_ct].items():
            row = {
                'rare_cell_type': rare_ct,
                'de_method': de_method,
            }
            
            # Add cell count statistics if available
            if rare_ct in rare_cell_stats:
                row['full_count'] = rare_cell_stats[rare_ct]['full']['count']
                row['full_pct'] = rare_cell_stats[rare_ct]['full']['pct']
                row['sps_count'] = rare_cell_stats[rare_ct]['sps']['count']
                row['sps_pct'] = rare_cell_stats[rare_ct]['sps']['pct']
                row['sps_enrichment'] = rare_cell_stats[rare_ct]['sps']['enrichment_vs_full']
                row['random_count'] = rare_cell_stats[rare_ct]['random']['count']
                row['random_pct'] = rare_cell_stats[rare_ct]['random']['pct']
                row['random_enrichment'] = rare_cell_stats[rare_ct]['random']['enrichment_vs_full']
            
            # Score metrics
            if 'score' in metrics:
                row['sps_mean_score'] = metrics['score']['sps_mean_score']
                row['random_mean_score'] = metrics['score']['random_mean_score']
                if metrics['score']['random_mean_score'] != 0:
                    row['score_improvement_pct'] = (
                        (metrics['score']['sps_mean_score'] - metrics['score']['random_mean_score']) 
                        / metrics['score']['random_mean_score'] * 100
                    )
                else:
                    row['score_improvement_pct'] = 0
            ...
            if 'pvalue' in metrics:
                ...
                if metrics['pvalue']['random_mean_neglogp'] != 0:
                    row['neglogp_improvement_pct'] = (
                        (metrics['pvalue']['sps_mean_neglogp'] - metrics['pvalue']['random_mean_neglogp']) 
                        / metrics['pvalue']['random_mean_neglogp'] * 100
                    )
                else:
                    row['neglogp_improvement_pct'] = 0
    ...
    # Create text summary
    summary_text = """
Statistical Power Analysis Summary (LCMV Dataset)
=================================================

Rare Cell Types Identified:
"""
    for rare_ct in rare_cell_types:
        if rare_ct in rare_cell_stats:
            stats = rare_cell_stats[rare_ct]
            summary_text += f"\n  {rare_ct}:\n"
            summary_text += f"    Full: {stats['full']['count']:,} cells ({stats['full']['pct']:.3f}%)\n"
            summary_text += f"    SPS:  {stats['sps']['count']:,} cells ({stats['sps']['pct']:.3f}%) [{stats['sps']['enrichment_vs_full']:.2f}x]\n"
            summary_text += f"    Random: {stats['random']['count']:,} cells ({stats['random']['pct']:.3f}%) [{stats['random']['enrichment_vs_full']:.2f}x]\n"
    
    summary_text += "\nKey Findings by DE Method:\n"
    
    for de_method in DE_METHODS:
        method_rows = df[df['de_method'] == de_method] if len(df) > 0 else pd.DataFrame()
        if len(method_rows) > 0:
            summary_text += f"\n  {de_method.upper()}:\n"
            for _, row in method_rows.iterrows():
                rare_ct = row['rare_cell_type']
                score_imp = row.get('score_improvement_pct', 0)
                neglogp_imp = row.get('neglogp_improvement_pct', 'N/A')
                summary_text += f"    {rare_ct}: Score improvement {score_imp:+.1f}%"
                if neglogp_imp != 'N/A':
                    summary_text += f", -log10(p) improvement {neglogp_imp:+.1f}%"
                summary_text += "\n"
...
def run_analysis(de_methods=None, top_n=50, logger=None):
    """
    Run the complete statistical power analysis for LCMV.
    ...
    if logger is None:
        logger = setup_logger('statistical_power_analysis_lcmv')
    ...
    logger.info("=" * 80)
    logger.info("Experiment 1: Statistical Power Analysis for Rare Cell Markers (LCMV)")
    logger.info("=" * 80)
    
    # Load dataset and identify rare cell types
    logger.info("\n" + "=" * 40)
    logger.info("Step 1: Loading data and identifying rare cell types")
    logger.info("=" * 40)
    ...
    try:
        ...
        # Compute statistics for rare cell types
        rare_cell_stats = compute_rare_cell_type_statistics(
            adata, sps_indices, random_indices, rare_cell_types, logger
        )
        
        # Save cell type analysis
        cell_type_df.to_csv(os.path.join(OUTPUT_DIR, 'cell_type_distances.csv'), index=False)
        
        # Free memory from large dataset
        del adata
        import gc
        gc.collect()
    ...
    if not rare_cell_types:
        ...
        # Fall back to frequency-only criterion
        rare_cell_types = cell_type_df[cell_type_df['frequency_pct'] < 2.0]['cell_type'].tolist()
    
    logger.info(f"\nWill analyze {len(rare_cell_types)} rare cell types: {rare_cell_types}")
    
    # Analyze each rare cell type and DE method
    all_results = {}
    all_tests = {}
    all_overlaps = {}
    ...
    for rare_ct in rare_cell_types:
        logger.info("\n" + "=" * 80)
        logger.info(f"Analyzing rare cell type: {rare_ct}")
        logger.info("=" * 80)
        
        all_results[rare_ct] = {}
        all_tests[rare_ct] = {}
        all_overlaps[rare_ct] = {}
        
        for de_method in de_methods:
            logger.info("\n" + "=" * 40)
            logger.info(f"DE Method: {de_method}")
            logger.info("=" * 40)
            
            try:
                # Load markers
                sps_markers, random_markers, full_markers = load_rare_cell_markers(
                    rare_ct, de_method
                )
                
                if len(sps_markers) == 0 or len(random_markers) == 0:
                    logger.warning(f"No markers found for {rare_ct} with {de_method}")
                    continue
                ...
                all_results[rare_ct][de_method] = metrics
                ...
                all_overlaps[rare_ct][de_method] = overlap
                
                # Statistical tests
                tests = statistical_tests(metrics, logger)
                all_tests[rare_ct][de_method] = tests
                
                # Create visualizations
                logger.info(f"\nCreating visualizations...")
                plot_pvalue_comparison(metrics, de_method, rare_ct, FIGURES_DIR)
                plot_volcano_comparison(sps_markers, random_markers, de_method, rare_ct, 
                                       FIGURES_DIR, top_n)
                plot_score_comparison(metrics, de_method, rare_ct, FIGURES_DIR)
                
            except FileNotFoundError as e:
                logger.warning(f"Marker gene results not found for {rare_ct} with {de_method}: {e}")
                logger.warning("Make sure to run the scanpy marker gene pipeline first!")
            except Exception as e:
                logger.error(f"Failed to analyze {rare_ct} with {de_method}: {e}")
                import traceback
                logger.error(traceback.format_exc())
    ...
    if all_results:
        plot_summary_comparison_all_rare_types(all_results, rare_cell_types, FIGURES_DIR)
        
        summary_df, summary_text = create_summary_table(
            all_results, rare_cell_stats, rare_cell_types, OUTPUT_DIR
        )
        logger.info("\n" + summary_text)
    ...
    results = {
        ...
        'rare_cell_stats': rare_cell_stats,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Statistical Power Analysis for Rare Cell Markers (LCMV)'
    )
    parser.add_argument(
        '--de-methods',
        type=str,
