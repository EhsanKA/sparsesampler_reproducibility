...
total_combinations=$(( ${#DATASETS[@]} * ${#SIZES[@]} * ${#REPS[@]} ))

echo "Submitting clustering visualization job array with $total_combinations combinations"
...
#SBATCH -J clustering_viz_array
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=${CLUSTER_EMAIL}
#SBATCH -o ${LOG_PATH}/clustering_visualization/logs/output_clustering_%A_%a.stdlog
#SBATCH -e ${LOG_PATH}/clustering_visualization/logs/output_clustering_%A_%a.stderr
#SBATCH -t 6:00:00
#SBATCH --mem=50G
#SBATCH -A ohler
#SBATCH -p normal
...
if [ "\$dataset" == "mcc" ]; then
    ...
    ref=34
    # Use an even higher Leiden resolution for LCMV to further increase the number of clusters
    resolution=1.5
else
...
output_file="${PROJECT_ROOT}/analysis/results/sampling_clustering/\${dataset}_size\${size}_rep\${rep}_metrics.csv"
...
# Debugging info
echo "=========================================="
echo "Clustering Visualization Task \$task_id"
echo "=========================================="
...
python visualize_sampled_clustering.py --dataset \$dataset --ref \$ref --size \$size --rep \$rep --resolution \$resolution
