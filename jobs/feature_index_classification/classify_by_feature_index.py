...
generated with different feature indices vs Random sampling.

Goal: Find which feature_index gives:
1. Good overall F1 score
2. Better or similar performance on Osteoblast (rare cell type)

Usage:
    python classify_by_feature_index.py --feature-index 18 --size 100000
"""
...
MCC_REF_PATH = os.path.join(DATA_PATH, "mcc/benchmark/30")
TEST_FEATURE_INDEX_PATH = os.path.join(DATA_PATH, "test_feature_index/mcc/30")
...
N_SEEDS = 5
N_TOP_GENES = 2000
REF = 30  # Reference size for MCC

# Cell types in order of frequency
CELL_TYPES = ['fibroblast', 'osteoblast', 'chondrocyte', 'mesodermal cell', 'lateral mesodermal cell']
...
def load_mcc_dataset():
    """Load MCC dataset from h5ad file."""
    print("Loading MCC dataset...")
    adata_path = os.path.join(MCC_REF_PATH, "adata.h5ad")
...
def load_random_indices(size=100000, rep=0):
    ...
    pkl_path = os.path.join(MCC_REF_PATH, f"random/{size}/{rep}/results.pkl")
...
def run_analysis(feature_index, size=100000, rep=0):
    ...
    for seed in range(N_SEEDS):
        ...
        print(f"  Osteoblast F1: {sps_result['per_class']['osteoblast']['f1']:.4f}")
        ...
        print(f"  Osteoblast F1: {random_result['per_class']['osteoblast']['f1']:.4f}")
    ...
    for method in ['sps', 'random']:
        ...
        summary[f'{method}_osteoblast_f1_mean'] = np.mean([r['per_class']['osteoblast']['f1'] for r in results])
        summary[f'{method}_osteoblast_f1_std'] = np.std([r['per_class']['osteoblast']['f1'] for r in results])
    ...
    summary['osteoblast_f1_improvement'] = (summary['sps_osteoblast_f1_mean'] - summary['random_osteoblast_f1_mean']) * 100
    ...
    print(f"\nOsteoblast F1:")
    print(f"  SPS:    {summary['sps_osteoblast_f1_mean']:.4f} ± {summary['sps_osteoblast_f1_std']:.4f}")
    print(f"  Random: {summary['random_osteoblast_f1_mean']:.4f} ± {summary['random_osteoblast_f1_std']:.4f}")
    print(f"  Improvement: {summary['osteoblast_f1_improvement']:+.2f}%")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Feature Index Classification Analysis'
    )
    parser.add_argument(
        '--feature-index',
        type=int,
