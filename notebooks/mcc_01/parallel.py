...
def generate_sps(adata, size, seed = 1234):
    return sample(X=adata.X, size=size, seed=seed, feature_index=12) 
...
def main(ref, method, size, rep, seed):
    ...
    args = {"adata": adata, "size": size, "seed": seed}
    method_dict = {
    "sps": (generate_sps, args),
    "hopper": (generate_hopper, args),
    "random": (generate_random, args),
    "scsampler": (generate_scsampler, args),
    }
